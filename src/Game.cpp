...
void Game::init() {
    ...
    /******** CUBEMAP AND ENVIRONMENT **********/
     ...
     //create the cubemap texture and assign everything to special vars in shader
    ...
    ECS.getComponentFromEntity<Transform>(sphere_entity).translate(1.5f, 0.0f, 0.0f);
    ...
    graphics_system_.getMaterial(sphere_mesh.material).shader_id = phong_shader->program;
    ...
    ECS.getComponentFromEntity<Transform>(ref_entity).translate(-1.5f, 0.0f, 0.0f);
    Mesh& ref_mesh = ECS.createComponentForEntity<Mesh>(ref_entity);
    ref_mesh.geometry = graphics_system_.createGeometryFromFile("data/assets/sphere.obj");

    //for material, we must set the reflection shader and the texture to be used as a cube_map
    ref_mesh.material = graphics_system_.createMaterial();
    graphics_system_.getMaterial(ref_mesh.material).shader_id = reflection_shader->program;
    graphics_system_.getMaterial(ref_mesh.material).cube_map = cubemap_texture;
    ...
    //load everything from scene parser
    //Parsers::parseJSONLevel("data/assets/reflection-level.json", graphics_system_, control_system_);
